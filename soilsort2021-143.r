...
#enhance resolution (90 m) tgs ----
tgs_zonal2 <- read.csv('fy2021-refresh/tgs_90m.csv')
mu_tgs <- merge(unique(mu[,c('lmapunitiid', 'dmuiid')]), tgs_zonal2, by.x =  'lmapunitiid', by.y = 'VALUE')
mu_tgs$tgswt <- mu_tgs$MEAN*mu_tgs$COUNT
mu_tgs <- aggregate(list(tgswt=mu_tgs$tgswt, wt=mu_tgs$COUNT), by=list(dmuiid =mu_tgs$dmuiid ), FUN='sum')
mu_tgs$tgs2 <- mu_tgs$tgswt/mu_tgs$wt
mu <-  merge(mu, mu_tgs[,c('dmuiid','tgs2')], by='dmuiid', all.x = TRUE)
...
# mlra.grp <- left_join(mlra.mu[,c('MLRARSYM','dmuiid','Count')], usergroup[,c('dmuiid','grpname')])
# s.groupmlra <- mlra.grp %>% group_by(MLRARSYM, grpname) %>% summarise(count=sum(Count))
# write.csv(s.groupmlra, 'data/s.groupmlra.csv', row.names = F)
...
lru.mu <- subset(lru.mu, select=c(dmuiid, MLRA, LRU))
...
#merge ----
compsorts <- merge(comp[,c('coiid', 'majcompflag','comppct_r' ,'compname', 'taxorder', 'taxsubgrp', 'taxclname', 'drainagecl' ,'slope_r', 'elev_r', 'maat_r')], fc.hz.carb.min, by='coiid', all.x = T )
...
compsorts <- merge(mu[,c('areasymbol','areaname','lmapunitiid','dmuiid','muiid','nationalmusym','muname','tgs','tgs2')], compsorts, by.x = 'dmuiid', by.y = 'dmuiid', all.y = F )
#fix pmorigin
...
#soil sort for MLRA 143 ----
s.reserve <- s
s <- s.reserve

s <- subset(s, majcompflag %in% 'TRUE' & MLRA %in% '143')
...
#2 Floodplains ----                
s$Site<-ifelse(s$Site %in% "Not",
               ifelse(s$muname %in% c("flooded") | s$compname %in% c("Alluvial land")|grepl("flood",s$landform_string)|s$flood %in% 'flood' | grepl('flood', s$landform_string),'F143XY110ME'
                      ,s$Site),s$Site)
s0 <- subset(s, s$Site %in% 'F143XY110ME') #replicate
s0$Site <- 'F143XY120ME';s <- rbind(s,s0) #name duplicate alternative ecoiid
...
s$Site<-ifelse(s$Site %in% "Open Wetlands", ifelse(grepl('saprists',s$taxclname), 'F143XY210ME',
               ifelse(grepl('euic',tolower(s$taxclname)) | (s$T50_pH > 4.5 & !grepl('dysic',tolower(s$taxclname))) ,'F143XY220ME','F143XY230ME')),s$Site) 
...
s$Site<-ifelse(s$Site %in% "Forested Wetlands",
               ifelse(s$taxorder %in% 'histosols','F143XY302ME',
                      ifelse((grepl('complex',s$muname) & s$muiid %in% verypoorlycomplex)|s$drainagecl %in% 'very poorly',
