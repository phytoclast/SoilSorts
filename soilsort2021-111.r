...
mlra <- read.csv('data/mlra2021.csv')
mlra <- subset(mlra, Count > 100000)
mu <- readRDS(file='fy2021-111/mu.RDS')
mlra$MLRA.total <- mlra$Count
mlra$MLRA.value <- mlra$Value
mlra.mu <- read.csv('data/mlra2021cob.csv')
mlra.mu <- merge(mlra[,c('MLRA.value','MLRARSYM', 'MLRA.total')], mlra.mu[,c('Value', 'Count', 'Feature_CONU1', 'ncnesoil30m')], by.x='MLRA.value', by.y='Feature_CONU1')
...
#saveRDS(comp, file='fy2021-111/comp.RDS')
comp <- readRDS("fy2021-111/comp.RDS")

#cm <- get_comonth_from_NASIS_db(fill = TRUE, SS=F)
#saveRDS(cm, file='fy2021-111/cm.RDS')
cm <- readRDS("fy2021-111/cm.RDS")
...
#saveRDS(fc, file='fy2021-111/fc.RDS')
fc <- readRDS(file='fy2021-111/fc.RDS')
